...
/**
 * @brief Print to stdout contents of __m256d vector spliting it up according to
 * T
 *
 * @tparam T
 * @param value
...
#define SUM(a, b, c) M == Method::Kahan ? ksum(a, b, c) : nsum(a, b, c);
	...
	if (M == Method::Neumaier) sumV[0] = _mm256_add_pd(sumV[0], cV[0]);
	...
	*/
	...
	// (Consider using compensated summation here, but we can probably assume
	// that all of our accumulators are similar magnitude at this point.)
...
#define SUM(a, b, c) M == Method::Kahan ? ksum(a, b, c) : nsum(a, b, c);
	...
	if (M == Method::Neumaier) sumV[0] = _mm512_add_pd(sumV[0], cV[0]);

	// Horizontally add the elements of sumV[0].
	// (Consider using compensated summation here, but we can probably assume
	// that all of our accumulators are similar magnitude at this point.) (Note:
	// this intrinsic doesn't map to a single instruction.)
...
500'000);
  ...
  std::cout << "naive                                " << naive(data, N) <<
std::endl; std::cout << "fast                                 " << fast<4>(data,
N) << std::endl; std::cout << "accurate<Method::Kahan>              " <<
accurate<Method::Kahan>(data, N) << std::endl; std::cout <<
"accurate<Method::Neumaier>           " << accurate<Method::Neumaier>(data, N)
<< std::endl; std::cout << "accurate<Method::LongDouble>         " <<
accurate<Method::LongDouble>(data, N) << std::endl; std::cout <<
"fastAccurate<Method::Kahan>          " << fastAccurate<Method::Kahan, 4>(data,
N) << std::endl; std::cout << "fastAccurate<Method::Neumaier>       " <<
fastAccurate<Method::Neumaier, 4>(data, N) << std::endl; std::cout <<
"fastAccurateAVX512<Method::Kahan>    " << fastAccurateAVX512<Method::Kahan,
4>(data, N) << std::endl; std::cout << "fastAccurateAVX512<Method::Neumaier> "
<< fastAccurateAVX512<Method::Neumaier, 4>(data, N) << std::endl;

  double noelim = 0.0;
  std::chrono::steady_clock::time_point begin =
std::chrono::steady_clock::now(); for (size_t i = 0; i < 2000; i++) {
	// noelim += naive(data, N);
...
"[ms]" << std::endl; std::cout << "Time difference = " <<
std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() <<
"[µs]" << std::endl; std::cout << "Time difference = " <<
std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count() <<
"[ns]" << std::endl; std::cout << noelim << std::endl;
}
*/
