...
*/
	...
	double SkewNormalPDF::EvalPDF(const double *xx, const double *par, const int component)
	{
		...
		if (skew == 0.0)
			...
			sn =  2.*ROOT::Math::gaussian_pdf(xm, sigma, 0.0)*ROOT::Math::normal_cdf(skew*xm/sigma,1.0, 0.0);
	...
	double DoubleSidedCrystalBallPDF::EvalPDF(const double *xx, const double *par, const int component)
	{
		double mean = par[0];
		double sigma = par[1];
