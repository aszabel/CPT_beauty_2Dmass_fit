...
sWeights::sWeights(const std::string& _path_data, const std::string& _outfilename,
				   const std::string& _outtreename)
	: path_data(_path_data), outfilename(_outfilename), outtreename(_outtreename) {};
...
void sWeights::get_sWeigths(const double res[], bool BBbar) {
	...
	if (BBbar)
		...
		plus_minus = "minus";
	TFile* outfile =
		new TFile(Form("%s_%s.root", outfilename.c_str(), plus_minus.c_str()), "recreate");
	...
	if (Nevents == -1) Nevents = ch.GetEntries();
	...
	for (int n = 0; n < ncontr; n++) {
		for (int j = 0; j < ncontr; j++) {
			...
			for (int e = 0; e < emax; e++) {
				...
				mb_like = B_PDFs[n]->EvalPDF(&mcorr,
											 &res[Config::nvar_all_md + Config::nvar_offset_mb[n]]);
	...
	for (int e = 0; e < nevents; e++) {
		...
		for (int i = 0; i < ncontr; i++) {
			md_like = D_PDFs[i]->EvalPDF(&mdass, &res[Config::nvar_offset_md[i]]);
			mb_like =
				B_PDFs[i]->EvalPDF(&mcorr, &res[Config::nvar_all_md + Config::nvar_offset_mb[i]]);
			sum_k += md_like * mb_like * frac[i] * nevents;
		}
