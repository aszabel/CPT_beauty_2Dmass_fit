...
ChebyshevPDF::ChebyshevPDF() { IntCheb = 1.0; }
...
void ChebyshevPDF::CalcIntegral(const double *par, double min, double max) {
	...
	IntCheb = (1.0 - a2) * max + 0.5 * a1 * max * max + 2. / 3. * a2 * max * max * max -
			  (1.0 - a2) * min - 0.5 * a1 * min * min - 2. / 3. * a2 * min * min * min;
}
double ChebyshevPDF::getIntegral() { return IntCheb; }

ExponentPDF::ExponentPDF() { IntExp = 1.0; }
...
double ExponentPDF::getIntegral() { return IntExp; }
}  // namespace cpt_b0_analysis
