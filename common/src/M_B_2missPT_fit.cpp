...
namespace cpt_b0_analysis
{
	...
	double SkewNormalPlusCBPlusDoubleGaussPDF::EvalPDF(const double *xx, const double *par, const int component)
	{
		...
		if (component == 0)
			...
			return (1 - f_gauss) * (1.0 - f_12) * SkewNorm.EvalPDF(xx, par1) + (1 - f_gauss) * f_12 * CB.EvalPDF(xx, par2) + f_gauss * Gauss.EvalPDF(xx, par3);
	}
	...
	double JohnsonPlusCBPlusDoubleGaussPDF::EvalPDF(const double *xx, const double *par, const int component)
	{
		double f_gauss = abs(par[0]);
		double f_12 = abs(par[1]);
		double par1[4];
		...
		if (component == 0)
			...
			return (1.0 - f_gauss) * (1.0 - f_12) * JSU.EvalPDF(xx, par1) + (1.0 - f_gauss) * f_12 * CB.EvalPDF(xx, par2) + f_gauss * Gauss.EvalPDF(xx, par3);
	}
