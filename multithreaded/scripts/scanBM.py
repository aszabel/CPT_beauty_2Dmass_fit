...
for n in range(N):
    ...
    run_all = scripts_path / "runBMall.sh"
...
for fit, data in results.items():
    fig = fit.replace("res", "B_M")
    best_fit_path = output_path / fit / "best"
    best_fit_path.mkdir(parents=True, exist_ok=True)
    best_run_path = scan_path / data["best_run"]
    best_result_path = best_run_path / "results" / (config["MC_directory_MB"] + "_binned" if config["binned"] else "_unbinned")
    best_figures_path = best_run_path / "results" / (config["MC_directory_MB"] + ("_binned" if config["binned"] else "_unbinned") + "_figures")
    ...
    good_result_path = good_run_path / "results" / (config["MC_directory_MB"] + "_binned" if config["binned"] else "_unbinned")
    good_figures_path = good_run_path / "results" / (config["MC_directory_MB"] + ("_binned" if config["binned"] else "_unbinned") + "_figures")
