...
void Dfit() {
	...
	ch.Add(
		"/mnt/home/aszabelski/data2016/strip28r1_Selection/"
		"tree_missPT_D_M_MagDown25102023_nomassDmuCut/"
		"selected_data2016MagDown.root");

	// TFile fileh("hDp_mass_mu_250bins_range1800_1940_22016magUp.root",
	// "recreate"); TFile
	// fileh("hDp_mass_mu_250bins_range1800_1940_2016magUp_tight_eta.root",
	// "recreate");
	double minx = 1800.;
	double maxx = 1940.;
	double miny = 2700.;
	...
	const int nsigBu = 6;
	TString name_sigBu[nsigBu] = {"sigma_sigBu",   "mean_sigBu",  "f12_sigBu",
								  "sigmaCB_sigBu", "alpha_sigBu", "n_sigBu"};
	...
	const int nB0D = 7;
	TString name_B0D[nB0D] = {"sigma_B0D", "mean_B0D", "sigmaCB_B0D", "f12_B0D",
							  "alpha_B0D", "n_B0D",	   "alphah_B0D"};
	...
	const int nBuD = 7;
	TString name_BuD[nBuD] = {"sigma_BuD", "mean_BuD", "sigmaCB_BuD", "f12_BuD",
							  "alpha_BuD", "n_BuD",	   "alphah_BuD"};
	...
	for (int i = 0; i < nBuD; i++) {
		min->SetVariable(nsigBu + nBs + nB0D + i, name_BuD[i].Data(), data_BuD[i],
						 err_BuD[i] + 1.0e-6);
	...
	TString var[nprm] = {"mean",	"sigma",  "norm",	"gnorm1", "gmean1",
						 "gsigma1", "gnorm2", "gmean2", "gsigma2"};
	...
	for (int i = 0; i < ncontr; i++) {
		for (int j = 0; j < nprm; j++) {
			min->SetVariable(28 + i * nprm + j, Form("%s_%s", name[i].Data(), var[j].Data()),
							 prev_min[i * nprm + j], dxx[i][j] + 1.e-6);
			if (dxx[i][j] == 0) {
