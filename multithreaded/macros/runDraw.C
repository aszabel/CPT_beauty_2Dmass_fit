void runDraw(string config_file, string output_dir = "results", string install_path = "../..") {
	TStopwatch t;
	t.Start();
	...
	gROOT->ProcessLine(Form(".x %s/multithreaded/macros/Draw_details.C(\"%s\", \"%s\")",
							install_path.c_str(), config_file.c_str(), output_dir.c_str()));
	t.Stop();
	t.Print();
}
