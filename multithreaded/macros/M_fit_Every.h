...
void M_fit_Every(const TString& path_results, const std::vector<int>& nvars, const double& minM,
	...
	for (auto& choice : Config::int_choose_fits) {
		...
		if (nentries > ch.GetEntries()) {
			std::cerr << "The value of 'nentries' exceeds the number of events "
						 "in the file."
					  << std::endl;
			return;
		}
			...
			if (int(charge) == Config::sign) {
				...
				switch (Config::int_mass_variable) {
					...
					case 4:	 // missPT
						...
						break;
					default:
						std::cerr << "Unsupported mass variable " << Config::mass_variable
								  << std::endl;
						return;
				}
			}
		...
		auto fchi2binned = [&PDFs, nvar, hist, nevents, nbins, minM, maxM, choice, ndim,
			...
			for (int bin = 1; bin <= nbins; bin++) {
				...
				double estim =
					bin_width * double(nevents) * PDFs[choice]->EvalPDF(&bincenter, param);
		...
		*/
		...
		auto funcDraw = [&PDFs, nvar, min, nevents, nbins, minM, maxM, choice, replaceIndexVect](
		...
		for (int i = 0; i < PDFs[choice]->getComponentCount(); i++) {
			auto funcDrawComponent = [&PDFs, nvar, min, nevents, nbins, minM, maxM, choice, i,
									  replaceIndexVect](double* x, double* par) -> double {
				double param[nvar];
				for (int ivar = 0; ivar < nvar; ivar++) {
					param[ivar] = par[ivar];
				}
			...
			TF1* tfc = new TF1((std::string("tfc_") + std::to_string(i)).c_str(), funcDrawComponent,
							   minM, maxM, nvar);
