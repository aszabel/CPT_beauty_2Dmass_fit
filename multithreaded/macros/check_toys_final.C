...
void check_toys_final(string results, int sign) {
	...
	for (int i = 1; i <= nfits; i++) {
		...
		ifstream resin(Form("%s/fit2D_%d/results_%d_3.txt", results.c_str(), i, sign));
		resin >> status >> chi2;
		...
		int n0 = (nfrac) * (7 + 8);
		if ((status == 0 || status == 1) && chi2 < min_chi2 && res[n0 + 1] < 0.2 &&
			res[n0 + 2] < 0.2 && res[n0 + 3] < 0.2 && res[n0 + 5] < 0.2) {	// && i!=10){
			min_chi2 = chi2;
			best = i;
		}
	}
	cout << best << endl;  //"  " << min_chi2 << endl;
						   /*ifstream input(Form("%s/fit2D_%d/results_1_3.txt", results.c_str(),
						   best)); input>>status >> chi2; int i=0; while (input>>x>>dx){ res[i] =
						   abs(x); dres[i] = dx; i++;
							   }
							   input.close();
						   ...
						   for (int ifrac=0; ifrac<nfrac; ifrac++){
							   cout << res[n0+ifrac] << " frac" << ifrac << " " << dres[n0+ifrac] <<
						   endl << endl;
...
void check_toys(){
	...
	for (int itoy=1; itoy<=ntoys; itoy++){
		...
		ifstream input(Form("toy_res/toy_%d/results/fit2D_%d/results_1_0.txt",
itoy, best)); int status; double chi2, x, dx; input>>status >> chi2; int i=0;
			while (input>>x>>dx){
					res[i] = abs(x);
			dres[i] = dx;
			i++;
