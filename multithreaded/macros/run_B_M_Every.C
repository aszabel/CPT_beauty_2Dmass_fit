void run_B_M_Every(string config_file, string install_path="../.."){
	TStopwatch t;
	t.Start();
	...
	gROOT->ProcessLine(Form(".x %s/multithreaded/macros/B_M_fit_Every.C(\"%s\")", install_path.c_str(), config_file.c_str()));
	t.Stop();
	t.Print();
}
