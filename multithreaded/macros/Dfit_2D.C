...
void Dfit_2D(int sign) {
	...
	ch.Add(
		"/home/szabelskia/LHCb/data2016/"
		"tree_missPT_D_M_MagDown25102023_nomassDmuCut/"
		"selected_data2016MagDown.root");
	...
	auto fchi2 = [&md_shape, &mb_shape, vect_2D, xx, dxx, xx_mcorr, dxx_mcorr,
				  min](const double *par) -> double {
		...
		for (auto &[mdass, mcorr] : vect_2D) {
			...
			for (int i = 0; i < ncontr; i++) {
				...
				if (i == 4)
					md_like = chebyshev(&mdass, &param[i * nvar_md]);
				else
					md_like = md_shape.func_full(&mdass, &param[i * nvar_md]);
		...
		for (int i = 0; i < ncontr; i++) {
			...
			for (int ivar = 0; ivar < nvar_md; ivar++) {
				// if (ivar==1) continue;
				if (dxx[i][ivar] != 0)
					chi2 += (xx[i][ivar] - param[i * nvar_md + ivar]) *
							(xx[i][ivar] - param[i * nvar_md + ivar]) /
							(dxx[i][ivar] * dxx[i][ivar]);	// use the results of MC fits
			}
		}
		for (int i = 0; i < ncontr; i++) {
			for (int ivar = 0; ivar < nvar_mb; ivar++) {
				// cout << min->VariableName(ncontr*nvar_md+i*nvar_mb+ivar) << "
				// " << xx_mcorr[i][ivar] << "  " << dxx_mcorr[i][ivar] << endl;
				if (dxx_mcorr[i][ivar] != 0)
					chi2 +=
						(xx_mcorr[i][ivar] - param[ncontr * nvar_md + i * nvar_mb + ivar]) *
						(xx_mcorr[i][ivar] - param[ncontr * nvar_md + i * nvar_mb + ivar]) /
						(dxx_mcorr[i][ivar] * dxx_mcorr[i][ivar]);	// use the results of MC fits
			}
		}
	...
	for (int i = 0; i < ncontr; i++) {
		for (int ivar = 0; ivar < nvar_md; ivar++) {
			min->SetVariable(i * nvar_md + ivar,
							 Form("%s_%s", name[i].Data(), varname_md[ivar].Data()), xx[i][ivar],
							 dxx[i][ivar] + 1.0e-11);
	...
	TString varname_mb[nvar_mb] = {"mean_rc",	  "sigma_rc",	"f12_gaus",	  "mean1_gaus",
								   "sigma1_gaus", "mean2_gaus", "sigma2_gaus"};

	for (int i = 0; i < ncontr; i++) {
		for (int ivar = 0; ivar < nvar_mb; ivar++) {
			min->SetVariable(ncontr * nvar_md + i * nvar_mb + ivar,
							 Form("%s_%s", name[i].Data(), varname_mb[ivar].Data()),
							 xx_mcorr[i][ivar], dxx_mcorr[i][ivar] + 1.0e-11);
	...
	// double frac_init[ncontr-1] = {0.657945, 0.901491, 0.968305, 0.527389,
	// 0.0034851};
	for (int i = 0; i < ncontr - 1; i++) {
		if (sign == 0)
			min->SetVariable((nvar_md + nvar_mb) * ncontr + i, Form("par_frac%d", i), frac_init[i],
							 0.01);
		else
			min->SetVariable((nvar_md + nvar_mb) * ncontr + i, Form("par_frac%d", i),
							 frac_init_plus[i], 0.01);
...
void Draw(const double *res, double nevents, TString name[], int sign) {
	...
	for (int i = 0; i < ncontr; i++) {
		...
		for (int ivar = 0; ivar < nvar_mb; ivar++) {
			param[ncontr * nvar_md + i * nvar_mb + ivar] =
				res[ncontr * nvar_md + i * nvar_mb + ivar];
		}
	}
	...
	double integ = 0.0;
	for (int i = 0; i < ncontr; i++) {
		auto wrap = [&md_shape, &mb_shape, nevents, param, i, &frac](double *x,
																	 double *par) -> double {
			double bin_widthx = (maxx - minx) / double(nbins);
			double bin_widthy = (maxy - miny) / double(nbins);
			double fval_md, fval_mb;
			if (i == 4)
				fval_md = chebyshev(x, &param[i * nvar_md]);
			else
				fval_md = md_shape.func_full(x, &param[i * nvar_md]);
	...
	for (int binx = 1; binx <= hist->GetNbinsX(); binx++) {
		for (int biny = 1; biny <= hist->GetNbinsY(); biny++) {
			...
			if (err == 0.0) continue;
			double diff = hist->GetBinContent(binx, biny) -
						  tf2_sum->Eval(hist->GetXaxis()->GetBinCenter(binx),
