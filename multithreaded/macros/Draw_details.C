...
void Draw_details(std::string config_file) {
	...
	if (Nentries == -1) Nentries = ch.GetEntries();
	...
	TH1D *h_MC_signal =
		new TH1D("h_tau_mc", "time signal MC vs sweighted", nbins, Config::tMin, Config::tMax);
	...
	if (Config::sign == 1)
		...
		sign_name = "minus";
	TFile weightFile_plus(Form("../toy_res/toy_%d/%s_%s.root", Config::randSeed,
							   fileWeightsName.c_str(), sign_name.c_str()),
						  "readonly");
	...
	chMC.Add(
		...
		"selectedMagDown_B2Dmunu_signal_taustrip_nomassDmuCut/"
		"selected_MCsignal25102023.root");
	...
	// double p_value = h_tau_signal_sweighted->KolmogorovTest(h_MC_signal,
	// "X");
	...
	for (int i = 0; i < Config::ncontr; i++) {
		...
		func2D[i] = new TF2(Form("tf2_%d", i), wrap, Config::minDM, Config::maxDM,
							Config::minBMcorr, Config::maxBMcorr, 0);
	...
	TF2 *tf2_sum2D = new TF2("tf2_sum2D", func_sum2D, Config::minDM, Config::maxDM,
							 Config::minBMcorr, Config::maxBMcorr, 0);
	...
	for (int binx = 1; binx <= hist2D->GetNbinsX(); binx++) {
		for (int biny = 1; biny <= hist2D->GetNbinsY(); biny++) {
			...
			if (err == 0.0) continue;
			double diff = hist2D->GetBinContent(binx, biny) -
						  tf2_sum2D->Eval(hist2D->GetXaxis()->GetBinCenter(binx),
										  hist2D->GetYaxis()->GetBinCenter(biny));
...
double LikelihoodRatioTest(TH1 *h_data, TH1 *h_mc) {
	...
	for (int i = 1; i <= h_data->GetNbinsX(); ++i) {
		...
		logL_data += n_obs > 0 ? n_obs * std::log(n_obs) - n_obs - std::lgamma(n_obs + 1) : -n_obs;
	}
