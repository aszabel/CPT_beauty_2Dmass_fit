...
void check_fits(int &best, int itoy, string results_dir, string results_num) {
	...
	for (int i = 1; i <= nfits; i++) {
		ostringstream os;
		os << "toy_res/toy_" << itoy << "/" << results_dir.c_str() << "/fit2D_" << i << "/results_"
		   << results_num.c_str() << ".txt";
		string str1 = os.str();
		ifstream resin(str1);
		resin >> status >> chi2;
		resin.close();
		if ((status == 0 || status == 1) && chi2 < min_chi2) {
			min_chi2 = chi2;
			best = i;
		}
	}
	cout << best << "  " << min_chi2 << endl;
}
void check_all_toys() {
	gStyle->SetOptFit(1);
	const int nfrac = 6;
	...
	for (int ifrac = 0; ifrac < nfrac; ifrac++) {
		for (int ires = 0; ires < nres; ires++) {
			...
			string str1 = os.str();
			pull[ifrac][ires] =
				new TH1D(str1.c_str(), "", 200, gen_frac[ifrac] - 0.03, gen_frac[ifrac] + 0.03);
		}
	}

	string results_num[nres] = {"0_0", "0_3"};
	// string results_num[nres] = {"0_0", "0_2", "0_3", "0_3"};
	// string results_dir[nres] = {"results_binned100kMU_2",
	// "results_binned100kMU_2", "results_unbinned100kMU_2"};
	...
	for (int i = 0; i < nres; i++) {
		...
		for (int ivar = 0; ivar < n0 + nfrac; ivar++) {
			tree[i]->Branch(Form("res%d", ivar), &res[ivar], Form("res%d/D", ivar));
		}
	...
	for (int itoy = 1; itoy <= ntoys; itoy++) {
		...
		for (int ires = 0; ires < nres; ires++) {
			...
			ostringstream os;
			os << "toy_res/toy_" << itoy << "/" << results_dir[ires].c_str() << "/fit2D_" << bestie
			   << "/results_" << results_num[ires].c_str() << ".txt";
			string str1 = os.str();
			best[ires] = bestie;
			// ifstream input(Form("toy_res/toy_%d/%s/fit2D_%d/results_%s.txt",
			// itoy, results_dir[ires].c_str(), best,
			// results_num[ires].c_str()));
			...
			// cout << "======================================\n";
			for (int ifrac = 0; ifrac < nfrac; ifrac++) {
				resfrac[ires][ifrac] = abs(res[n0 + ifrac]);
				pull[ifrac][ires]->Fill(abs(res[n0 + ifrac]));
				// cout << res[n0+ifrac] << " frac" << ifrac << " " <<
				// dres[n0+ifrac] << endl << endl;
			}
			cout << res[n0] << " frac0\n";
			toynum[ires] = itoy;
			tree[ires]->Fill();
	...
	for (int ifrac = 0; ifrac < nfrac; ifrac++) {
		for (int ires = 0; ires < nres; ires++) {
			...
			ostringstream os;
			os << "plots/frac" << ifrac << "_" << results_dir[ires].c_str() << "_"
			   << results_num[ires].c_str() << ".pdf";
			string str1 = os.str();
			// c[ifrac][ires]->SaveAs(TString::Form("plots/frac%d_%s_%s.pdf",
			// ifrac, results_dir[ires].c_str(), results_num[ires].c_str()));
