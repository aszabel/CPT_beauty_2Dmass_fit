...
int main(int argc, char *argv[])
{
	...
	auto fchi2 = [&md_shape, &mb_shape, vect_2D, xx, dxx, xx_mcorr, dxx_mcorr](const double *par) -> double
	{
		...
		std::cout<<std::endl;
		*/
		std::cout<<"Params: "<<param[0]<<", "<<param[1]<<std::endl;
		...
		/*
		#pragma omp parallel shared(chi2)
		{
	...
	*/
	for(int ivar=ncontr*nvar_md; ivar<ncontr*(nvar_md+nvar_mb)+ncontr-1-5; ivar++){
		min->FixVariable(ivar);
	}

	/*
	double param[ncontr * (nvar_md + nvar_mb) + ncontr - 1] = {8.76823, 1870.23, 5.88311, 0.991988, 1.75894, 1.22521, 2.12646, 8.10586, 1870.23, 5.81046, 0.97345, 1.47844, 2.24577, 1.88047, 104.183, 1881.37, 12.3602, 0.268843, 0.293315, 8.69493, 1.8627, 8.66617, 1870.23, 5.66482, 0.99005, 1.90499, 0.808728, 1.9522, 1.88518e-08, 1.31834e-07, 1, 1, 1, 1, 1, 8.71697, 1870.23, 5.52935, 0.981435, 2.27281, 0.477669, 2.35198, 5944.83, 1092.15, 0.999998, 5183.81, 640.576, 5704.53, 1100, 5160.56, 1611.07, 0.999998, 4872.55, 788.412, 5204.5, 806.058, 5944.83, 1092.15, 0.999998, 5183.81, 640.576, 5704.53, 1100, 5944.83, 1092.15, 0.999998, 5183.81, 640.576, 5704.53, 1100, 4647.08, 2253.99, 0.99135, 5872.82, 1606.04, 5698.65, 613.154, 5944.83, 1092.15, 0.999998, 5183.81, 640.576, 5704.53, 1100, 0.468636, 0.736568, 0.973902, 0.949009, 0.0};

	TCanvas* c = new TCanvas();
	TH1D* h_chi2 = new TH1D("h_chi2", "Chi^2 + 116643.795363158", 100, 0.0, 116643.7953631582-116643.795363158);
	for(int i=0; i<1000; i++){
		//param[ncontr * (nvar_md + nvar_mb) + ncontr - 2] += 1e-10;
		double res = fchi2((double*)&param);
		h_chi2->Fill(res - 116643.795363158);
	}
	h_chi2->Draw();
	//h_chi2->GetXaxis()->SetNdivisions(2);
	c->SaveAs("chi2.pdf", "pdf");
	*/
