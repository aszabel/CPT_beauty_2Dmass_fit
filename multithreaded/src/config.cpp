...
int Config::load(const std::string& filename) {
	...
	if (config.contains("massVariable")) {
		...
		if (dictionaryChooseMassVariable.find(mass_variable) != dictionaryChooseMassVariable.end()) {
			int_mass_variable = dictionaryChooseMassVariable.at(mass_variable);
		} else {
			std::cerr << "Mass var '" << mass_variable << "' not found. Chose '2D', '1D_DM', '1D_BM'" << std::endl;
			return 1;
		}
		if (int_category > 0)
			is2D = false;
