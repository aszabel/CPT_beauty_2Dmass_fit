	...
	for (auto& int_choose_fit : Config::int_choose_fits) {
		...
		/*		for (int i = 0; i < Config::ncontr; i++)
				{
					...
					{
						min->SetVariable(Config::ncontr * Config::nvar_md + i * Config::nvar_mb +
		   ivar, (TString::Format("%s_%s", Config::contrName[i].c_str(),
		   Config::varname_mb[ivar].c_str())).Data(), Config::MC_MB[i][ivar],
		   Config::dMC_MB[i][ivar] + 1.0e-11); if (int_choose_fit == 0 || int_choose_fit == 2){
								min->FixVariable(Config::ncontr * Config::nvar_md + i *
		   Config::nvar_mb + ivar);
						}
						if (start_scratch)
		...
		*/
		...
		for (int i = 0; i < Config::ncontr; i++) {
			...
			// -1.0, 1.0); if (int_choose_fit == 1){
			if (i != 4) {
				min->SetVariableValue(Config::nvar_all_md + Config::nvar_all_mb + i,
									  0.0);
				min->FixVariable(Config::nvar_all_md + Config::nvar_all_mb + i);
			}
			if (start_scratch)
		...
		for (int ivar=0; ivar<(Config::nvar_md+Config::nvar_mb) * Config::ncontr + Config::ncontr;
		ivar++){
		...
		for (int icontr=0; icontr<Config::ncontr; icontr++){
			frac_st[icontr] = starting_point[(Config::nvar_md+Config::nvar_mb) * Config::ncontr +
		icontr]; if (Config::randSeed > -1 &&
		!min->IsFixedVariable((Config::nvar_md+Config::nvar_mb) * Config::ncontr + icontr) &&
		int_choose_fit==0){ double random = rand.Uniform(-1.0, 1.0); frac_st[icontr] = random;
			}
		}
		...
		if (Config::start_from_previous){
			...
			for (int ic=0; ic<Config::ncontr; ic++){
				double random = 0.0;
				if (Config::randSeed > -1 && !min->IsFixedVariable((Config::nvar_md+Config::nvar_mb)
		* Config::ncontr+ic) && int_choose_fit==3) random = rand.Uniform(-1.0, 1.0);
				frac_st[ic]=result0[(Config::nvar_md+Config::nvar_mb) *
		Config::ncontr+ic]*(1.0+0.001*random);
			}
		}
		...
		*/
		...
		auto fchi2 =
			wrap_chi2(D_PDFs, B_PDFs, vect_2D, Config::MC_MD, Config::dMC_MD, Config::MC_MB,
					  Config::dMC_MB, replaceIndexVect, int_choose_fit, hist1D);
		...
		results << min->X()[Config::nvar_offset_md[4] + 0] << "  "
				<< min->Errors()[Config::nvar_offset_md[4] + 0] << std::endl;
		results << min->X()[Config::nvar_offset_md[4] + 1] << "  "
				<< min->Errors()[Config::nvar_offset_md[4] + 1] << std::endl;
		results << min->X()[Config::nvar_all_md + 4] << "  "
				<< min->Errors()[Config::nvar_all_md + 4] << std::endl;
		...
		D_PDFs[4].get()->CalcIntegral(&min->X()[Config::nvar_offset_md[4]],
		...
		D_PDFs[4].get()->CalcIntegral(&min->X()[Config::nvar_offset_md[4]], Config::minDM, Config::maxDM);
...
std::function<double(const double*)> wrap_chi2(
	...
	*/
		...
		for (int i = 0; i < Config::ncontr; i++) {
			...
			// B_PDFs_get[i]->CalcIntegral(&param[Config::ncontr * Config::nvar_md + i *
			// Config::nvar_mb], Config::minBMcorr, Config::maxBMcorr);
		}
		/*
		   ...
		   (MC_MD[i][ivar] - param[i * Config::nvar_md + ivar]) / (2.0*(dMC_MD[i][ivar] *
		   dMC_MD[i][ivar])); // use the results of MC fits chi2 += 2.5e3*tmp;
							}
						}
					}
				}
					...
					for (int i = 0; i < Config::ncontr; i++)
				{
						...
						{
							if (dMC_MB[i][ivar] != 0){
								double tmp = (MC_MB[i][ivar] - param[Config::ncontr *
		   Config::nvar_md + i * Config::nvar_mb + ivar]) * (MC_MB[i][ivar] - param[Config::ncontr *
		   Config::nvar_md + i * Config::nvar_mb + ivar]) /(2.0* (dMC_MB[i][ivar] *
		   dMC_MB[i][ivar])); // use the results of MC fits
		...
		if (Config::binned) {
			...
			for (int bin_md = 1; bin_md <= nbins_md; bin_md++) {
				...
				for (int i = 0; i < Config::ncontr; i++) {
					...
					// double mb_val = B_PDFs_get[i]->EvalPDF(&mcorr, &param[Config::ncontr *
					// Config::nvar_md + i * Config::nvar_mb]);
				...
				if (err != 0.0) {
					...
					// std::cout << bin_md << "   "  << bin_mb << "  " << cont << "  " << sum_contr
