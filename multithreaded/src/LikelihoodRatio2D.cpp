...
int main(int argc, char* argv[]) {
	...
	try {
		if (argc != 3) {
			std::cerr << "Usage: LikeRatio config.json seed" << std::endl;
			return 1;
		}
	...
	std::vector<double>* tauvec = nullptr;
	ch.SetBranchAddress("vec_Tau", &tauvec);
	ch.GetEntry(0);
	const int nbins = 40;
	...
	if (nentries < 0) nentries = (*tauvec).size();
	if (nentries > (*tauvec).size()) {
	...
	int itry = 1;
	int loop_count = 0;
	std::cout << Config::int_choose_fits.size() << " int chooose \n";
	for (auto& int_choose_fit : Config::int_choose_fits) {
		...
		while ((itry <= Config::ntries || !goodfit) && itry <= 100) {
			...
			min->SetTolerance(Config::tolerance[loop_count - 1]);
			...
			double* pfrac = &result0[Config::ncontr * (Config::nvar_md + Config::nvar_mb)];
			...
			for (int i = 0; i < Config::ncontr; i++) {
				for (int ivar = 0; ivar < Config::nvar_md; ivar++) {
					param[i * Config::nvar_md + ivar] = result0[i * Config::nvar_md + ivar];
					if (ivar == 1 && i != 2 && i != 4)
						param[i * Config::nvar_md + ivar] = result0[1];
				}
				for (int ivar = 0; ivar < Config::nvar_mb; ivar++) {
					param[Config::ncontr * Config::nvar_md + i * Config::nvar_mb + ivar] =
						result0[Config::ncontr * Config::nvar_md + i * Config::nvar_mb + ivar];
				}
			}

			for (int i = 0; i < Config::ncontr; i++) {
				D_PDFs_get[i]->CalcIntegral(&param[i * Config::nvar_md], Config::minDM,
											Config::maxDM);
				B_PDFs_get[i]->CalcIntegral(
					&param[Config::ncontr * Config::nvar_md + i * Config::nvar_mb],
					Config::minBMcorr, Config::maxBMcorr);
			}

			for (int i = 0; i < Config::ncontr; i++) {
				auto wrap = [&D_PDFs_get, &B_PDFs_get, &param, i, &frac](double* x,
																		 double* par) -> double {
					...
					return frac[i] * fval_md * fval_mb;
				};
				func2D[i] = new TF2(Form("tf2_%d", i), wrap, Config::minDM, Config::maxDM,
									Config::minBMcorr, Config::maxBMcorr, nall);
			}
			auto func_sum2D = [&func2D, &param](double* x, double* par) -> double {
				double sum = 0.0;
				for (int i = 0; i < Config::ncontr; i++) {
					func2D[i]->SetNpx(300);
					func2D[i]->SetNpy(300);
					func2D[i]->SetParameters(param);
					sum += func2D[i]->Eval(x[0], x[1]);
				}
				return sum;
			};

			TF2 tf2_sum2D("tf2_sum2D", func_sum2D, Config::minDM, Config::maxDM, Config::minBMcorr,
						  Config::maxBMcorr, nall);
			tf2_sum2D.SetNpx(300);
			tf2_sum2D.SetNpy(300);

			tf2_sum2D.SetParameters(param);

			if (int(vect_2D.size()) == 0) {
				TRandom randgen(1000 + toyseed);
				for (int e = 0; e < nentries; e++) {
					double md, mb;
					md = mb = 0.0;
					tf2_sum2D.GetRandom2(md, mb, &randgen);
					vect_2D.push_back(std::make_pair(md, mb));
				}
			}
