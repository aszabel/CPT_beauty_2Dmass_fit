...
int main(int argc, char* argv[]) {
	...
	for (auto& int_choose_fit : Config::int_choose_fits) {
		...
		while ((itry <= Config::ntries || !goodfit) && itry <= 100) {
			...
			double* pfrac = &result0[Config::nvar_all_md + Config::nvar_all_mb];
			...
			for (int i = 0; i < Config::ncontr; i++) {
				for (int ivar = 0; ivar < Config::nvar_md[i]; ivar++) {
					param[Config::nvar_offset_md[i] + ivar] =
						result0[Config::nvar_offset_md[i] + ivar];
					if (ivar == 1 && i != 2 && i != 4)
						param[Config::nvar_offset_md[i] + ivar] = result0[1];
				}
				for (int ivar = 0; ivar < Config::nvar_mb[i]; ivar++) {
					param[Config::nvar_all_md + Config::nvar_offset_mb[i] + ivar] =
						result0[Config::nvar_all_md + Config::nvar_offset_mb[i] + ivar];
				}
			}
			...
			for (int i = 0; i < n_all; i++) {
				if (i != Config::nvar_all_md + Config::nvar_all_mb + 1) {
					...
					for (int j = 0; j < Config::ncontr; j++) {
						if (j == 1) continue;
						sumfrac += abs(min->X()[Config::nvar_all_md + Config::nvar_all_mb + j]);
					}
