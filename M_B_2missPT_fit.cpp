...
namespace cpt_b0_analysis{
   ...
   mb_2misspt_fit::mb_2misspt_fit(double _minn, double _maxx):minn(_minn), maxx(_maxx){ 
	}
double mb_2misspt_fit::func_full(const double *xx, const double *par){
		...
		auto gaus = [this](const double *x, const double *par)->double{
			...
			gaus1*=abs(norm1);
                        if(int_gaus1!=0) gaus1/=int_gaus1;
			...
			gaus2*=abs(norm2);
			if(int_gaus2!=0.0) gaus2/=int_gaus2;
                        return (gaus1+gaus2);
                };
