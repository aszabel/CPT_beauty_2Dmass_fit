...
void draw_results() {
	...
	/*
		...
		pdfs PDFs;
			auto func_conv = [&PDFs, r_bin_center, r_bin_cont, Nrbins] (double x, const double *par,
	   bool bbar) -> double{ double value =0.0; double Tau = x; for (int r_bin=1; r_bin<=Nrbins;
	   ++r_bin){ double r_rec = r_bin_center[r_bin-1]; double n_r = r_bin_cont[r_bin-1];

							double time_c =Tau-r_rec; //time of mean exp value in bin
							if (time_c<Params::tMin_gen || time_c> Params::tMax_gen ) continue;
							if (bbar)
									value += PDFs.time_acceptance(&time_c,
	   &par[4])*PDFs.func1D_f(&time_c, par)*n_r; else value += PDFs.time_acceptance(&time_c,
	   &par[7])*PDFs.func1D_fbar(&time_c, par)*n_r;
					}
			return value;
			};
					...
					TH1D *hist_resBbar = new TH1D("hist_resBbar", "", Nbins, Params::tMin,
	   Params::tMax); double normbar = 0.0; for (int bin=1; bin<=Nbins; bin++){ double t =
	   hist_Bbar->GetBinCenter(bin); double expect = func_conv(t, output, false);
	...
	*/
	...
	for (int bin = 1; bin <= pull_B->GetNbinsX(); bin++) {
		...
		// logHist_resB->SetBinContent(bin,
		// logHist_resB->GetBinWidth(bin)*logHist_resB->GetBinContent(bin));
		double expect = logHist_resB->GetBinContent(bin);
		double cont = logHistB->GetBinContent(bin);
		double err = logHistB->GetBinError(bin);
		if (err != 0) pull_B->SetBinContent(bin, (expect - cont) / err);
	}
	...
	for (int bin = 1; bin <= pull_Bbar->GetNbinsX(); bin++) {
		...
		if (err != 0) pull_Bbar->SetBinContent(bin, (expect - cont) / err);
	}
...
TH1D *make_log_bins(TH1D *hAsym) {
	...
	for (int i = 0; i <= nLogBins; ++i) {
		logBins[i] = TMath::Power(
			10, (i * (TMath::Log10(xMax) - TMath::Log10(xMin)) / nLogBins) + TMath::Log10(xMin));
	}

	// Create the new histogram with logarithmic bins
	TH1D *logHist =
		new TH1D("logHist", "Rebinned Histogram with Logarithmic Bins", nLogBins, logBins);
