...
std::function<double(const double *)> wrap_chi2(
	TH1D *logHist_expectB, TH1D *logHist_expectBbar, TH1D *logAsym, TH1D *logExpAsym,
	std::function<double(double, const double *, bool)> func_conv, std::vector<double> vec_time,
	std::vector<double> vec_time_bar, TH1D *hist_B, TH1D *hist_Bbar, TH1D *logHistB,
	TH1D *logHistBbar, int ii, std::vector<std::pair<int, int>> replaceIndexVect);
...
int main(int argc, char *argv[]) {
	...
	auto func_acc = [&PDFs](double *x, double *par) -> double {
		return PDFs.time_acceptance(x, par);
	};
	...
	for (int i = 0; i <= Config::nlogBins; ++i) {
		logBins[i] = TMath::Power(
			10, (i * (TMath::Log10(Params::tMax) - TMath::Log10(Params::tMin)) / Config::nlogBins) +
					TMath::Log10(Params::tMin));
	}
	...
	for (int i = 0; i < tree_r->GetEntries(); i++) {
		tree_r->GetEntry(i);
		if (tau < Params::tMin || tau > Params::tMax) continue;
		if (charge) {
	...
	TFile weightFile_plus(Form("../multithreaded/%s_plus.root", fileWeightsName.c_str()),
						  "readonly");
	...
	TFile weightFile_minus(Form("../multithreaded/%s_minus.root", fileWeightsName.c_str()),
						   "readonly");
...
*/
	...
	auto func_conv = [&PDFs, r_bin_center, r_bin_cont, Nrbins, Apol, Bpol, Cpol](
						 double x, const double *par, bool bbar) -> double {
		double value = 0.0;
		double Tau = x;
		for (int r_bin = 1; r_bin <= Nrbins; ++r_bin) {
			double r_rec;
			...
			double time_c =
				r_rec * (Apol * Tau * Tau + Bpol * Tau + Cpol);	 // time of mean exp value in bin
			...
			if (bbar)
				...
				value +=
					PDFs.time_acceptance(&time_c, &par[7]) * PDFs.func1D_fbar(&time_c, par) * n_r;
		}
		return value;
	};
	...
	// double params[10] = {0.0, imz_gen, 0.657552, 0.997031, -0.250837, 1.10646, 0.056071,
	// -0.250837, 1.10646, 0.056071};
		...
		for (int itry = 0; itry < Config::ntries[ii]; itry++) {
			...
			*/
			...
			for (const auto &rep_var : replace_var) {
				...
				auto it = std::find(Config::fixVect[ii].begin(), Config::fixVect[ii].end(),
									rep_var.first);
				if (it == Config::fixVect[ii].end()) Config::fixVect[ii].push_back(rep_var.first);
			...
			min->SetErrorDef(TMath::ChisquareQuantile(
				CL_normal, Config::nvar_time - Config::fixVect[ii].size()));
			std::cout << Config::fixVect[ii].size() << std::endl;

			auto func_chi2 = wrap_chi2(logHist_expectB, logHist_expectBbar, logAsym, logExpAsym,
									   func_conv, vec_time, vec_time_bar, hist_B, hist_Bbar,
									   logHistB, logHistBbar, make_asym, replaceIndexVect);
			...
			for (int i = 0; i < Config::nvar_time; i++) output[i] = out[i];
			const double *err = min->Errors();
			for (int i = 0; i < Config::nvar_time; i++) error[i] = err[i];
	...
	for (int int_bbar = 1; int_bbar < 2; int_bbar++) {
		for (int bin = 1; bin <= Config::nlogBins; bin++) {
			...
			if (int_bbar == 0) {
				...
				logHist_expectBbar->SetBinContent(bin,
												  expect * logHist_expectBbar->GetBinWidth(bin));
...
std::function<double(const double *)> wrap_chi2(
	...
	std::vector<double> vec_time_bar, TH1D *hist_B, TH1D *hist_Bbar, TH1D *logHistB,
	TH1D *logHistBbar, int ii, std::vector<std::pair<int, int>> replaceIndexVect) {
	auto func_chi2 = [logHist_expectB, logHist_expectBbar, logAsym, func_conv, vec_time,
					  vec_time_bar, hist_B, hist_Bbar, logHistB, logHistBbar, ii,
		...
		for (int int_bbar = 0; int_bbar < 2; int_bbar++) {
			...
			for (int bin = 1; bin <= Config::nlogBins; bin++) {
				if (int_bbar == 1) integral = integralBbar;
				...
				if (!make_asym) {
					...
					if (err != 0) chi2 += (cont - expect) * (cont - expect) / err / err;
				}
				if (int_bbar == 0) {
			...
			if (make_asym) {
				...
				// logHist_expectBbar->Scale(logHistBbar->Integral()/logHist_expectBbar->Integral());
				TH1D *logExpAsym =
					dynamic_cast<TH1D *>(logHist_expectB->GetAsymmetry(logHist_expectBbar));
				for (int bin = 1; bin <= Config::nlogBins; bin++) {
		...
		return chi2 /
			   double(Config::nlogBins - Config::nvar_time + Config::fixVect[ii].size() - 1.0);
	};
	return func_chi2;
}
