...
std::vector<std::shared_ptr<PDFInterface>> Config::getVectorPDFs(const std::string& domain) {
	...
	if (domain == std::string("Dmass")) {
		for (auto intshape : intshapesDM) {
			switch (intshape) {
				...
				case 1:
					...
					break;
				default:
					std::cerr << "Error while DM_pdf dynamic declaration. Check if shapes from "
								 "config file refer to the shapes defined in the code."
							  << std::endl;
		...
		for (auto intshape : intshapesBMcorr) {
			switch (intshape) {
				case 0:
					...
					break;
				default:
					std::cerr << "Error while BM_pdf dynamic declaration. Check if shapes from "
								 "config file refer to the shapes defined in the code."
							  << std::endl;
					return std::vector<std::shared_ptr<PDFInterface>>{};
			}
		...
		std::cerr << "Error wrong domaine in getVectorPDFs(domain) chose 'Dmass' or 'Bmass'"
				  << std::endl;
		return std::vector<std::shared_ptr<PDFInterface>>{};
	}
	return vPDFs;
...
int Config::load(const std::string& filename) {
	...
	if (config.contains("functionCalls")) {
		...
		if (functionCalls < 0.) {
			std::cerr << "Invalid config file: 'functionCalls' must be a positive integer."
					  << std::endl;
			return 1;
		}
	} else {
	...
	if (config.contains("randSeed")) {
		...
		if (randSeed < -1) {
			std::cerr << "Invalid config file: 'randSeed' must be -1, 0 or a positive integer."
	...
	if (config.contains("input_file")) {
		...
		if (stat(input_file.c_str(), &sb) != 0) {
			std::cerr << "Invalid config file: File " << input_file << " does not exist."
					  << std::endl;
			return 1;
		}
	} else {
	...
	if (config.contains("r_factor_file")) {
		...
		if (stat(r_factor_file.c_str(), &sb) != 0) {
			std::cerr << "Invalid config file: File " << r_factor_file << " does not exist."
					  << std::endl;
			return 1;
		}
	} else {
	...
	if (config.contains("BMcorr_range")) {
		...
		if (minBMcorr < 0.0 || maxBMcorr < 0.0 || minBMcorr > maxBMcorr) {
			std::cerr << "Invalid config file: interval 'BMcorr_range' must be positive."
					  << std::endl;
			return 1;
		}
	} else {
	...
	if (config.contains("DMshapes")) {
		...
		if (int(DMshapes.size()) != ncontr) {
			std::cerr << "Invalid config file: Number of given DM pdfs does not match ncontr. "
					  << std::endl;
			return 1;
		}
	} else {
	...
	if (config.contains("BMshapes")) {
		...
		if (int(BMshapes.size()) != ncontr) {
			std::cerr << "Invalid config file: Number of given BM pdfs does not match ncontr. "
					  << std::endl;
			return 1;
		}
	} else {
