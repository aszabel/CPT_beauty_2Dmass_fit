...
public:
	Params() {}
	/*
	Params(constexpr double Gamma_, constexpr double dGamma_, constexpr double dM_, double Rez_,
	double Imz_, double omega2_, double eta_, double A_D_, double A_dir_, double sigma_, int N_, int
	Nbar_, int Nr_, int Nrbins_, int npar_, double alpha_, double beta_, double t_shift_, bool
	time_acc_, bool conv_): Gamma(Gamma_), dGamma(dGamma_), dM(dM_), Rez(Rez_), Imz(Imz_),
	omega2(omega2_), eta(eta_), A_D(A_D_), A_dir(A_dir_), sigma(sigma_), N(N_), Nbar(Nbar_),
	Nr(Nr_), Nrbins(Nrbins_), npar(npar_), alpha(alpha_), beta(beta_), t_shift(t_shift_),
...
*/
	...
	static constexpr int N =
		0;	// int(((1.0 - global.bkg_sb_percent)*global.signal_percent)*double(global.N));
	static constexpr int Nr = 10000;
	static constexpr int Nbar =
		0;	// int(((1.0-global.bkg_sb_percent)*global.signal_percent)*double(global.Nbar));
...
/*  Params b0_pars;
  ...
  -0.0006, 0.0, 0.0,
  b0_pars.sigma,
  int(global.bkg_Bu_percent*b0_pars.N), int(global.bkg_Bu_percent*b0_pars.Nbar), b0_pars.Nr,
  b0_pars.Nrbins, 3, //acceptance the same as for singnal b0_pars.alpha, b0_pars.beta,
  b0_pars.t_shift, true, true);
*/
